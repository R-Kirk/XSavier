...
class Chart(QtChart.QChart):
    def __init__(self, data, min_x, max_x, min_y, max_y):
        super().__init__()
        self.objectName = "Chart_Monthly"
        self.series = QtChart.QLineSeries()
        self.series.setName(f"Yearly_Cashflow")

        ##### - SERIES 1 - Get points for Zero Profit Line
        for x, y in data:
            self.series.append(qtc.QPointF(int(x), y))
        ...
        self.axis_y.setMin(min_y - 1000.00)
        self.axis_y.setMax(max_y + 1000.00)
        ...
        self.setTitle(f"Yearly Cashflow")
